...
[dates,pos,tot,dth] = get_DNC("FL")
dth_FL = [0]*(len(dth)-1)
for i in range(len(dth)-1): dth_FL[i] = dth[i]-dth[i+1]
[dates,pos,tot,dth] = get_DNC("TX")
dth_TX = [0]*(len(dth)-1)
for i in range(len(dth)-1): dth_TX[i] = dth[i]-dth[i+1]
[dates,pos,tot,dth] = get_DNC("AZ")
dth_AZ = [0]*(len(dth)-1)
for i in range(len(dth)-1): dth_AZ[i] = dth[i]-dth[i+1]
[dates,pos,tot,dth] = get_DNC("CA")
dth_CA = [0]*(len(dth)-1)
for i in range(len(dth)-1): dth_CA[i] = dth[i]-dth[i+1]
l = min(len(dth_FL),len(dth_TX),len(dth_AZ),len(dth_CA))
dth_sum = []
...
if True:
    ...
    #plt.ylim([0,3500])
    plt.legend(['FL+TX+CA+AZ'])
    plt.tight_layout()
    plt.savefig('US_Death_4states',dpi=150)
plt.close()
...
if True:
    ...
    plt.legend(['FL+TX+CA+AZ'])
    plt.tight_layout()
    plt.savefig('US_Death_4states_JHU',dpi=150)
plt.close()
